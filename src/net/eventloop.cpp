...
#define Delete_To_Epoll()\
    ...
    //需要判断当前线程是否有 Eventloop，如果有的话需要报错,这里不太理解是什么意思：没问题，比如你主线程需要创建一个Eventloop事件，通过getCurrentEventloop
    //得到t_current_eventloop，前提是你没有其他的t_current_eventlop
    ...
    void Eventloop::loop(){
        ...
        while(!m_stop_flag){
            ...
            int rt = epoll_wait(m_epoll_fd, result_events,g_epoll_max_events,timeout);  //有一个问题，第一步进入，对于主线程来说，他已经添加三个事件了，这三个事件分别是将wakeupfd,timefd,listenfd添加到树上，还没有触发这个事件。所以现在只有一个，因为其他事件还没有触发
            ...
            if(rt < 0){
                ...
                for(int i = 0; i < rt; i++){
                    ...
                    //     ERRORLOG("socket [%d] occur other unknow event",fd_event->getFd());
                    //     deleteEpollEvent(fd_event);
                    // }
                    if(trigger_event.events & EPOLLIN){  //这里啥意思两任何一个等于1都可以，我怎么感觉这块应该是&
                        DEBUGLOG("fd %d trigger EPOLLIN event", fd_event->getFd());
                        addTask(fd_event->handler(FDEvent::IN_EVENT));  //返回fd_event->handler(FDEvent::IN_EVENT)可读函数，添加到任务队列中
    ...
    bool Eventloop::isInloopThread(){
        DEBUGLOG("now time threadid %d, and this thread id %d",getThreadId(),m_thread_id);
    ...
    Eventloop* Eventloop::getCurrentEventloop(){
        if(t_current_eventlop){
            return t_current_eventlop;
        }
        t_current_eventlop = new Eventloop();
        return t_current_eventlop;
    }
